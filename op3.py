...
class KMeans_cu():
    ...
    def kernel3(self):
        ...
        __device__ int barrier0[%(N)s/%(size)s/10/%(K)s + 1];
        __device__ int barrier1[%(N)s/%(size)s/10/%(K)s + 1];
        __device__ int barrier2[%(N)s/%(size)s/10/%(K)s + 1];

        __device__ int flag=1;
        __device__ float oldmu[%(K)s * 2];
        ...
        float *newmu, unsigned int *num) {
            ...
            while(flag==1){
                ...
                barrier0[blockIdx.x] = 0;
                if (lid==0)
                    barrier1[blockIdx.x] = 1;
                if (lid<%(N)s/%(size)s/10/%(K)s + 1)
                    while(!barrier1[lid])
                ...
                while(id < N)
                {
                    point[0] = data[2*id];
                    point[1] = data[2*id + 1];
                    float tmp = sqdist(point[0], oldmu[0], point[1], oldmu[1]);
                    int key = 0;
                    for(int i=1; i < K; i++){
                        if(sqdist(point[0], oldmu[2*i], point[1], oldmu[2*i+1])<tmp){
                            tmp = sqdist(point[0], oldmu[2*i], point[1], oldmu[2*i+1]);
                ...
                barrier2[blockIdx.x] = 0;
                if (lid==0)
                    barrier0[blockIdx.x] = 1;
                if (lid<%(N)s/%(size)s/10/%(K)s + 1)
                    while(!barrier0[lid])
                __syncthreads();                        //global sync
                ...
                barrier1[blockIdx.x] = 0;
                if (lid==0)
                    barrier2[blockIdx.x] = 1;
                if (lid<%(N)s/%(size)s/10/%(K)s + 1)
                    while(!barrier2[lid])
                __syncthreads();                        //global sync
